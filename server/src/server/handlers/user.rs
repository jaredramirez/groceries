...
pub fn user_all(req: &mut Request) -> IronResult<Response> {
    ...
    get_new_response(status::Ok, Some(serde_json::to_string(&users).unwrap()))
}
...
pub fn user_save(req: &mut Request) -> IronResult<Response> {
    ...
    if let Ok(None) = req_body_result {
        return get_new_response(status::NotModified, Some("No request body found!".to_string()));
    } else if let Err(err) = req_body_result {
        return get_new_response(status::NotModified, Some(err.to_string()));
    }
    ...
    get_new_response(status::NoContent, None)
}
...
pub fn user_by_id(req: &mut Request) -> IronResult<Response> {
    ...
    if let Err(e) = id_result {
        return get_new_response(status::NotFound, Some(e.to_string()))
    }
    ...
    if let Some(user_bson) = result {
        ...
        return get_new_response(status::Ok, Some(serde_json::to_string(&user).unwrap()));
    }

    get_new_response(status::NotFound, Some("Failed to get User!".to_string()))
}

fn get_new_response(status_code: status::Status, data: Option<String>) -> IronResult<Response> {
    if let Some(message) = data {
        return Ok(Response::with((status_code, message)))
    } else {
        return Ok(Response::with((status_code)))
    }
}
